...
func TestDiskCache_Compaction(t *testing.T) {
	...
	// Note: With write-once semantics, updates are now no-ops
	// This section doesn't create fragmentation anymore
	for i := 0; i < 25; i++ {
		key := []byte(fmt.Sprintf("key_%d", i))
		value := []byte(fmt.Sprintf("updated_value_%d", i))
		err = cache.Set(key, value) // Will be no-op since keys already exist
		if err != nil {
	...
	// Perform compaction (disk-usage-based, no age limit)
	_, err = cache.Compact(0)
	if err != nil {
	...
	for i := 0; i < 25; i++ {
		...
		// With write-once semantics, expect original value, not updated value
	...
	// Verify original keys (25-35 were not deleted with write-once)
	for i := 25; i < 50; i++ {
		key := []byte(fmt.Sprintf("key_%d", i))
		value, err := cache.Get(key)
		if err != nil {
...
func BenchmarkDiskKV_CompactionSpeed(b *testing.B) {
	...
	for i := 0; i < 1000; i++ {
		...
		// Note: With write-once semantics, these are now no-ops
		if i%3 == 0 {
...
// BenchmarkDiskCache_LoadHintsFromSegment benchmarks the performance of loading hints from segment files.
// This measures the CPU and memory overhead of reconstructing the in-memory keydir from hint data
// during cache startup/recovery operations.
//
// The benchmark tests various scenarios:
//   - Small (1k entries): Baseline performance with small datasets
//   - Medium (10k entries): Typical workload size
//   - Large (50k entries): Large dataset to measure scalability
//   - MultiSegment (30k/3seg): Multiple segment files to measure file I/O overhead
//   - LargeKeys (10k): Impact of key size on memory allocations
//
// Usage examples:
//
//	# Run all variants with default settings
//	go test -bench=BenchmarkDiskCache_LoadHintsFromSegment -benchmem
//
//	# Run specific variant with more iterations
//	go test -bench=BenchmarkDiskCache_LoadHintsFromSegment/Large -benchtime=10x -benchmem
//
...
//   - ns/op: Time to load all hints per iteration
//   - B/op: Bytes allocated per operation
//   - allocs/op: Number of allocations per operation
//   - keys_loaded: Total keys loaded (custom metric for verification)
func BenchmarkDiskCache_LoadHintsFromSegment(b *testing.B) {
	// Test with different entry counts to see how performance scales
	testCases := []struct {
		name        string
		numEntries  int
		numSegments int
		...
		{"Small_1k_entries", 1000, 1, 20, 100},
		{"Medium_10k_entries", 10000, 1, 20, 100},
		{"Large_50k_entries", 50000, 1, 20, 100},
		{"MultiSegment_30k_3seg", 30000, 3, 20, 100},
		{"LargeKeys_10k", 10000, 1, 100, 200},
	}

	for _, tc := range testCases {
		b.Run(tc.name, func(b *testing.B) {
			tmpDir, err := os.MkdirTemp("", "bitcask_hints_bench")
			if err != nil {
				b.Fatalf("Failed to create temp dir: %v", err)
			}
			defer os.RemoveAll(tmpDir)
			...
			// Calculate entries per segment
			entriesPerSegment := tc.numEntries / tc.numSegments
			if tc.numSegments > 1 && entriesPerSegment == 0 {
				entriesPerSegment = 1
			}

			// Write test data
			for i := 0; i < tc.numEntries; i++ {
				// Create keys with specified size
				keyBase := fmt.Sprintf("benchmark_key_%0*d", tc.keySize-20, i)
				if len(keyBase) > tc.keySize {
					keyBase = keyBase[:tc.keySize]
				}
				key := []byte(keyBase)

				// Create values with specified size
				...
				if tc.numSegments > 1 && (i+1)%entriesPerSegment == 0 && i+1 < tc.numEntries {
					if err := cache.rotateLogFile(); err != nil {
						b.Fatalf("Failed to rotate log file: %v", err)
					}
				}
			}
			...
			// Force rotation to ensure hints are written for all segments
			if err := cache.rotateLogFile(); err != nil {
				b.Fatalf("Failed to rotate log file: %v", err)
			}
			...
			// Get list of segment files for benchmarking
			files, err := filepath.Glob(filepath.Join(tmpDir, "*.log"))
			if err != nil {
				b.Fatalf("Failed to list log files: %v", err)
			}
			if len(files) == 0 {
				b.Fatal("No log files found")
			}

			// Store file info for benchmark
			...
			for _, filename := range files {
				// Open temporarily to read header
				file, err := os.Open(filename)
				if err != nil {
					b.Fatalf("Failed to open log file: %v", err)
				}

				// Create a temporary cache just to read header
				tempCache, _ := NewDiskCache(tmpDir, ByteSliceMarshaler{})
				header, err := tempCache.readFileHeader(file)
				file.Close()
				tempCache.Close()

				if err != nil {
					b.Fatalf("Failed to read file header: %v", err)
				}

				if header.hintOffset > 0 {
					fileID, err := parseFileID(filename)
					if err != nil {
						b.Fatalf("Failed to parse file ID: %v", err)
					}

					fileInfos = append(fileInfos, fileInfo{
						path:       filename,
						fileID:     fileID,
						hintOffset: header.hintOffset,
					})
				}
			}

			if len(fileInfos) == 0 {
				b.Fatal("No files with hints found")
			}

			b.ResetTimer()
			b.ReportAllocs()

			// Benchmark the hint loading
			for i := 0; i < b.N; i++ {
				// Create a minimal cache instance for each iteration
				// Use a separate temp dir to avoid conflicts
				iterTmpDir, err := os.MkdirTemp("", "bitcask_hints_iter")
				if err != nil {
					b.Fatalf("Failed to create iter temp dir: %v", err)
				}

				testCache, err := NewDiskCache(iterTmpDir, ByteSliceMarshaler{})
				if err != nil {
					os.RemoveAll(iterTmpDir)
					b.Fatalf("Failed to create test cache: %v", err)
				}

				// Load hints from each segment file - this is what we're measuring
				for _, info := range fileInfos {
					...
					err = testCache.loadHintsFromSegment(file, info.hintOffset, info.fileID)
					file.Close()

					if err != nil {
						testCache.Close()
						os.RemoveAll(iterTmpDir)
						b.Fatalf("Failed to load hints: %v", err)
					}
				}
			...
			// Report the final memory and key count stats
			finalCache, _ := NewDiskCache(tmpDir, ByteSliceMarshaler{})
			if finalCache != nil {
				stats := finalCache.Stats()
				b.ReportMetric(float64(stats.Keys), "keys_loaded")
				finalCache.Close()
			}
		})
	}
...
func TestAutoCompactionDisabled(t *testing.T) {
	// This test is kept for backwards compatibility but auto-compaction
	// is no longer part of the write-once design.
	// Use Compact(maxAge) for age-based or disk-usage-based segment removal instead.
	t.Skip("Auto-compaction not implemented in write-once design")
}
...
func TestDiskCache_TruncateCorruptedSegment(t *testing.T) {
	...
	// Use Stats() to check how many keys were recovered into the keydir
	stats := cache.Stats()
	scannedCount := int(stats.Keys)
...
func TestDiskCache_TruncateMiddleCorruption(t *testing.T) {
	...
	// Count recovered keys - use Stats() to check keydir
	stats := cache.Stats()
...
func TestDiskCache_NoTruncateOnValidFile(t *testing.T) {
	...
	// Verify all keys are still there - use Stats() to check keydir
	stats := cache.Stats()
	count := int(stats.Keys)
	uniqueKeys := count // Stats.Keys gives us the unique key count

	if uniqueKeys != 10 {
		t.Errorf("Expected 10 unique keys, got %d", uniqueKeys)
	}

	t.Logf("File correctly not truncated: %d bytes, %d total entries, %d unique keys", newSize, count, uniqueKeys)
}
